...
## BPT-14: タスク初期値マスタと自動生成（2026-08-25）

> 旧 BPT-12（ContractTasks / InsuredTaskState）は**廃止**。固定4タスクは汎用 `Task[]` に一本化された（ADR-TASK-MASTER 参照）。

1. 管理者/役員でログイン → 設定画面の「タスク初期値マスタ」（または /admin?tab=task_templates）を開く。
2. トリガー（世帯作成時 / 案件作成時 / 商品追加時 / ステージ到達時）を選び、既定タスクを追加・編集・削除する。
3. 案件を新規作成する→ opportunity_created トリガーの既定タスクが自動生成される（`sourceMasterId` 付き）。
4. 商品を追加する→ product_added トリガー（商品カテゴリ別）のタスクが自動生成される。
5. 自動生成されたタスクは**削除不可**（削除ボタンなし）だが、担当/期限/優先度/メモ/done の編集は可。手動追加タスク（sourceMasterId なし）は削除可。
6. 商品を削除する→ 当該 product スコープタスクが連動削除される。

## BPT-15: 日報一覧・過去ブロック閲覧・日付ナビ（2026-08-25・D1/D2/D3）

1. （D3）/nippou →「日報一覧」タブで過去日報を検索。general は自分の日報のみ、manager/executive/admin は全員の日報が見えることを確認。
2. （D1）過去日報のタイムラインブロックをクリック→ 詳細（read-only）が表示され、商談/報告へのリンクを辿れることを確認（編集不可）。
3. （D2）日付「←/→」で日報のない日（土日等）をスキップして前/次の日報に移動すること、旧「前の日報/次の日報」ボタンが削除されていることを確認。

## BPT-16: IA再編（メニュー5項目）の導線（2026-08-25）

1. サイドメニューが①ダッシュボード ②商談一覧 ③顧客一覧 ④日報 ⑤設定 の5項目（ロール限定項目は後続）であることを確認。
2. ダッシュボード内の上部タブで「ダッシュボード/営業実績」を切替でき、どちらも画面幅を使う全幅レイアウトであることを確認（DESIGN_GUIDE 準拠）。
3. 顧客一覧で「世帯/契約」タブ、日報で「日報/カレンダー/日報一覧」タブが切替できることを確認。
4. 旧URL（/sales-perf, /today, /calendar, /households, /policies, /search）への直接アクセスが適切にリダイレクトされることを確認。検索窓がヘッダにあることを確認。

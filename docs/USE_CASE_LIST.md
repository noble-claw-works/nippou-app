...
| UC-G-23 | **日報一覧検索・詳細関閉**(日堃3点D3・2026-08-25) | 過去日報を月単位で検索・関閉。詳細詳風（read-only） | ① /nippou → "日報一覧" タブ ② 日付範囲を絞り込み ③ 過去日報をクリックで詳細関閉(詳鞠を通じて知らぬ情報を関閉で確認。ただし署別不可) | /nippou(日報一覧タブ) | 種別不可: general=自分、manager=全師一覧、executive=全師一覧、admin=全師一覧 |
| UC-G-24 | **日報内過去ブロック詳細閉じ**(日堃3点D1・2026-08-25) | 日報上の過去実績ブロックをクリックして签別モードで詳細関閉 | ① /nippou(当日日報)の実績タイムライン ② 過去日昨日等の履歴ブロックをクリック ③ 詳細関閉モード(read-only): 詳細署名メモ不可 | /nippou | general以上 |
| UC-G-25 | **日付ナビ統一**(日堃3点D2・2026-08-25) | 日報上の日付前後ナビゲーションを統一 | ① /nippou上で日報有り日のみをスキップし箖手ナビを履歴ブロック等の日官加を加を沙署す。上粗沙署て不整合を保つ | /nippou | general以上 |

...
- **編集/削除**: 編集（担当/期限/優先度/メモ/done）は手動・自動生成いずれも可。**削除は手動追加タスク（`sourceMasterId===undefined`）のみ可。自動生成タスク（`sourceMasterId!=undefined`）は削除不可**（主上決裁 2026-08-25 [要確認2]）。UI 上は自動生成タスク行の削除ボタンを非表示/無効化する。
...
## 7. 未決事項 / リスク（[要確認]） — **主上決裁 2026-08-25 で1〜5確定**

- **[要確認1] 証券回収タスク → salesPerf 連携** → ✅**決裁: 連携なし**（主上 2026-08-25「連携なし。情報として調べられれば大丈夫」）。証券回収は通常タスクとして完了管理し、salesPerf ファネルへは流さない。本ADRのまま（連携なし）で確定。
- **[要確認2] 手動削除タスクの再生成抑止** → ✅**決裁: 自動生成タスクは削除不可**（主上 2026-08-25）。すなわち `sourceMasterId != undefined`（自動生成）のタスクは UI から削除できない仕様にする（§4.1 の「編集/削除いずれも可」は自動生成タスクの**削除のみ不可**へ変更・下記 §4.1 改訂参照）。よって「削除→再発火→再生成」問題自体が発生しない（削除できないため冪等判定だけで足りる）。編集（担当/期限/優先度/メモ/done）は自動生成タスクも可。手動追加タスク（sourceMasterId=undefined）は従来どおり削除可。
- **[要確認3] 商品削除時の product タスク** → ✅**決裁: 連動削除**（主上 2026-08-25）。`ProposalProduct` 削除時、その `productId` を持つ `scope='product'` タスクは done/未done を問わず**連動削除**する（履歴保持はしない）。§5.4 の設計提案どおり確定。
- **[要確認4] 「意向シート/署名」の被保険者粒度廃止の妥当性** → ✅**決裁: タスク化がベター**（主上 2026-08-25）。被保険者ごとの分割はやめ、意向シート/署名は**案件スコープの通常タスク**として扱う（確定要件1どおり案件1件集約）。朱雀UX検証で運用確認する。
- **[要確認5] マスタ「セット」概念のUI表現** → ⏸**決裁: 後で検討**（主上 2026-08-25）。T4 は**単票CRUDで先行実装**する。セット複製・カテゴリ一括付替等の一括操作UIは本スプリントに含めず、後日別要件として検討する。
- **[要確認6] 世帯タスクの横断統合ビュー**: 未決（本スプリント対象外・需要が出たら別途）。
...
- 実装は T1〜T5 の5段階。未決5点は **主上決裁 2026-08-25 で全て確定**（§7）: ①salesPerf連携なし ②**自動生成タスクは削除不可**（編集は可・手動追加タスクは削除可） ③商品削除時は product タスク連動削除 ④意向シート/署名は案件スコープの通常タスク化 ⑤マスタUIは単票CRUDで先行（一括操作は後日）。これにより T2 以降を確定し実装着手する。

...
import type { ProposalProduct, ProposalRound } from "../types";
...
export function OpportunityDetailPage() {
  ...
  // req7: タスク対応
  const taskRows = getOppTaskRows(opp, persons);
  const doneTaskCount = taskRows.filter((r) => r.done).length;

  const handleTaskToggle = (key: string, done: boolean) => {
    if (key === "policyCollect") {
      updateOpportunity(id!, togglePolicyCollect(opp, done));
    } else if (key === "policyReview") {
      updateOpportunity(id!, togglePolicyReview(opp, done));
    } else if (key.startsWith("intentSheet:")) {
      const personId = key.slice("intentSheet:".length);
      updateOpportunity(id!, toggleIntentSheet(opp, personId, done));
    } else if (key.startsWith("signature:")) {
      const personId = key.slice("signature:".length);
      updateOpportunity(id!, toggleSignature(opp, personId, done));
    }
  };
  ...
  const TABS: { key: Tab; label: string }[] = [
    ...
    { key: "tasks", label: `☑️ タスク (${doneTaskCount}/${taskRows.length})` },
  ...
  return (
    <div className="p-4 max-w-4xl mx-auto">
      ...
      {tab === "tasks" && (
        <div className="bg-white rounded-xl border border-gray-200 p-5">
          <h2 className="font-semibold text-gray-800 mb-4">タスク管理</h2>
          {taskRows.length === 0 ? (
            <p className="text-sm text-gray-400 text-center py-4">
              タスクは設定されていません（対象被保険者またはは契約情報を登録すると表示されます）
            </p>
          ) : (
            <div className="space-y-1">
              {/* セクションヘッダー：案件単位 */}
              ...
              {taskRows
                .filter((r) => r.scope === "opportunity")
                .map((row) => (
                  <label
                    key={row.key}
                    className="flex items-center gap-3 py-2.5 px-1 rounded-lg hover:bg-gray-50 cursor-pointer"
                  >
                    <input
                      type="checkbox"
                      checked={row.done}
                      onChange={(e) =>
                        handleTaskToggle(row.key, e.target.checked)
                      }
                      className="w-4 h-4 accent-blue-500 rounded"
                    />
                    <span className="flex-1">
                      <span
                        className={`text-sm ${row.done ? "line-through text-gray-400" : "text-gray-800"}`}
                      >
                        {row.label}
                      </span>
                      {row.done && row.date && (
